...
import shutil # Added for the delete_model function
    ...
    if request.method == "POST":
        try:
            ...
            if graph_type == "distance":
                ...
                # Need to handle case where df_proc only has one sample (shouldn't happen here)
                coords = df_proc.values 
                if coords.shape[0] > 1:
                    ...
                    plt.text(0.5, 0.5, "PCA requires more than 1 sample.", ha='center', va='center')
                    plt.title("PCA Plot Not Available")
...
def admin():
    if request.method == "POST":
        ...
        try:
            ...
            # 🛑 REMOVED REDUNDANT MANUAL CLEANING!
            # The cleaning/imputation/encoding is now fully handled by preprocess_fit_save
